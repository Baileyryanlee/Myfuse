...
static int ResolveMeleeHitBaseDamage(Actor* victim, ColliderInfo* atInfo, ColliderInfo* acInfo, const char* srcLabel,
    ...
    if (swordBaseDamage > 0) {
        Fuse::Log("[FuseDBG] FreezeDamageProbe: src=%s victim=%p item=%d mat=%d base=%d d0=%d d1=0x%08X d2=%d\n",
                  srcLabel ? srcLabel : "unknown", (void*)victim, itemId, static_cast<int>(materialId), swordBaseDamage,
...
void OnFrame_Objects_Pre(PlayState* play) {
    ...
    if (gHammerizeAppliedFrame != -1 && static_cast<int64_t>(play->gameplayFrames) > gHammerizeAppliedFrame) {
        RestoreSwordBaseDmgFlags(player);
    ...
    if (hammerLevel > 0 && Fuse::IsSwordFused() && IsPlayerSwingingSword(player) &&
    ...
    if (isHammerAttacking && IsHammerGroundImpactFrame(player)) {
        ...
        if (fused) {
            ...
            if (poundLevel > 0) {
                ...
                Fuse::Log("[FuseDBG] HammerPoundUp: event=ground-impact item=hammer mat=%d lvl=%u dura=%d/%d "
                          "note=no-radius\n",
