...
void DrawDisplayListSegment06(PlayState* play, s32 objIdx, uintptr_t restoreSeg06, Gfx* dl) {
    if (play == nullptr || objIdx < 0 || dl == nullptr) {
        return;
    }
    ...
    gSPSegment(POLY_OPA_DISP++, 0x06, (uintptr_t)play->objectCtx.status[objIdx].segment);
    gSPMatrix(POLY_OPA_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_OPA_DISP++, dl);
    gSPSegment(POLY_OPA_DISP++, 0x06, restoreSeg06);
